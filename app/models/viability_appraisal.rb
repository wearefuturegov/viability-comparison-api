class ViabilityAppraisal < ApplicationRecord
  ...
  scope :commercial, -> (value) { value.downcase == "true" ? where.not(commercial_area_square_centimetres: 0) : (value.downcase == "false" ? where(commercial_area_square_centimetres: 0) : all) }
